...
static void sbus_work_handler(struct k_work *work)
{
	...
	if (buf->ts == 0
		...
		|| (flags & SBUS_FLAGS_FAILSAFE && !config->continue_on_fale_safe)
		) {
		unlock_buffer(buf);
		return;
	}
	...
	for (int i = 0; i < config->num_channels; i++) {
		int ret = input_report_abs(data->dev, config->channels[i].axis, values[i],
					(i == config->num_channels -1 ? true : false), K_FOREVER);
		if (ret)
...
static int sbus_receiver_init(const struct device *dev) {
    ...
    if (ret != 0)
        return ret;
#endif
    LOG_INF("Initializing SBUS Receiver!");
    data->dev = dev;
...
#define SBUS_RC_INIT(i) \
    ...
    static struct sbus_config sbus_config_##i = { \
        ...
        .continue_on_fale_safe = DT_INST_PROP(i, continue_on_fale_safe), \
    }; \
