...
static inline int channel_set(const struct device *dev, uint8_t value)
{
...
{
	const struct servo_data *p_data = dev->data;
...
static int pwm_servo_init(const struct device *dev)
{
	...
	if (!pwm_is_ready_dt(&p_cfg->pwm_spec)) {
		LOG_ERR("PWM device %s not ready", p_cfg->pwm_spec.dev->name);
		return -ENODEV;
	}

	err = channel_set(dev, p_cfg->init_value);
	if (err) {
		LOG_ERR("channel_set error %d", err);
		return err;
	}
...
#define SERVO_DEVICE(n) \
	static const struct servo_cfg servo_cfg_##n = { \
		.init_value   = DT_PROP(DT_DRV_INST(n), init_value), \
		.min_pulse_us = DT_PROP(DT_DRV_INST(n), min_pulse_us), \
		.max_pulse_us = DT_PROP(DT_DRV_INST(n), max_pulse_us), \
		.pwm_spec     = PWM_DT_SPEC_GET(DT_DRV_INST(n)) \
	}; \
